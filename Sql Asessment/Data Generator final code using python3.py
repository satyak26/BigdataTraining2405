...
data_table1 = {
    ...
    "state_code": [random.choice(list(us_states.keys())) for _ in range(1000)],  # Valid U.S. state codes
}
...
# Task 2: Create table2 with intentional mismatches
data_table2 = {
    # Mismatch: Use a random state_code unrelated to table1
    "state_code": [random.choice(list(us_states.keys())) for _ in range(1000)],
    # Mismatch: Generate new random names unrelated to table1
    "name": [random_string(3) for _ in range(1000)],
    "County": [random.choice(us_states[state]['counties']) for state in df_table1["state_code"]],  # Still match counties with valid state codes
    "order_date": [random_date(2020, 2024) for _ in range(1000)],
    "phone": df_table1["phone"],  # Keep phone consistent between tables
